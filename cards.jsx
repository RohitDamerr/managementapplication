...
function Cards({ addTask, setInputDiv, data, setUpdatedData }){
  const headers = {
  ...
  return(
    <div className="grid grid-cols-3 gap-4 p-4">
      {data && 
        data.map((items, i) => (
          <div className="flex flex-col justify-between bg-gray-800 rounded-sm p-4">
            {/* ye div banaye hai taki h3 or p ek me rhe */}
            <div>
              <h3 className="text-xl font-semibold">{items.title}</h3>
              <p className="text-gray-300 my-2">{items.desc}</p>
            </div>

            {/* ye div banaye button ke liy alg se ye rhe taki dono ko phir arrange kr ske flex lga ke*/}
            <div className="mt-4 w-full flex items-center">
              <button className={`${items.complete ? "bg-green-700": "bg-red-500"} p-2 rounded w-3/6`}
            ...
            <div className="text-white p-2 w-3/6 text-2xl font-semibold flex justify-around">
              ...
              <button onClick={() => handleImportant(items._id)}>{items.important === false ? (<CiHeart/>) : (<FaHeart className="text-red-600"/>)}</button>
      ...
      {/* Add Task ke liy  agr props me true aayega tb hi add task dikhayega otherwise nhi*/}
      {addTask === "true" && 
